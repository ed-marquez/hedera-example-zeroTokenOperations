...
export default class ContractHelper {
	...
	getResultValidator(stepIndex) {
		...
		return (/** @type {hashgraph.ContractFunctionResult} */ result) => {
			const responseStatus = hashgraph.Status._fromCode(result.getInt32(0));
			const isValid = responseStatus == hashgraph.Status.Success;
			if (!isValid) {
				console.log(`Encountered invalid response status ${responseStatus.toString()}`);
			}
			return isValid;
		};
	}
	...
	async executeSteps(firstStepToExecute, lastStepToExecute, signer) {
		for (let stepIndex = firstStepToExecute; stepIndex <= lastStepToExecute; stepIndex++) {
			console.log(`Attempting to execute step ${stepIndex}`);

			let transaction = new hashgraph.ContractExecuteTransaction().setContractId(this.contractId).setGas(10_000_000);
			...
			if (feePayerAccountId != null) {
				transaction.setTransactionId(hashgraph.TransactionId.generate(feePayerAccountId));
			}
			...
			transaction = /** @type {hashgraph.ContractExecuteTransaction} */ (await transaction.signWithSigner(signer));
			...
			if (this.getResultValidator(stepIndex)(functionResult)) {
				console.log(`step ${stepIndex} completed, and returned valid result. (TransactionId "${record.transactionId.toString()}")`);
			} else {
